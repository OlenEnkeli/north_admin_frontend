<script lang="ts">
    ...
    export let openModal: (action: string, index?: number) => undefined;

</script>

<div class="panel__main__header text-xs font-semibold uppercase text-neutral-100 flex">
    <div class="flex-auto mr-5 text-gray-900 rounded">
        <div class="mx-5 flex justify-start py-2 items-center">
            ...
            <div class="flex-auto">
                <select
                    bind:value={itemsOnPage}
                    on:change={getItems}
                    class="font-bold border-0 text-xs"
                >
                    {#each itemsOnPageOptions as itemOnPageOption}
                        <option value={itemOnPageOption}>
                            {itemOnPageOption}
                        </option>
                    {/each}
                </select>
            ...
            <span class="pl-1 flex-auto font-bold">{items ? items.totalAmount : 0}</span>
        </div>
    </div>

    <div class="flex-auto mr-5 text-gray-900 rounded">
        <div class="mx-5 h-full flex justify-start py-2 items-center">
            <input
                bind:checked={includeSoftDeleted}
                on:click={getItems}
                id="default-checkbox"
                type="checkbox"
                class="flex-auto"
    ...
    <Button
        title="Create"
        color="blue"
        on:click={() => {openModal('create', undefined)}}
        classes="flex-auto uppercase mr-5"
    >
        <CirclePlusSolid />
    </Button>
...
<!--    <button on:click={() => {openModal('create', undefined)}} class="flex-auto mr-5 panel__main__header__create bg-cyan-800 hover:bg-cyan-900 py-2 px-4 rounded inline-flex items-center">-->
